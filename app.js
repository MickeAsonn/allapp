...
/* Tankning/Tvätt export (sorted by Datum+Tid) */
...
/* Körjournal med start/slut datum+tid */
...
function App(){
  ...
  function addTrip(){
    if(!jForm.startDate || jForm.fromKm==='' || jForm.toKm===''){ alert('Fyll startdatum, från km och till km'); return; }
    if(km(jForm.toKm) < km(jForm.fromKm)){ alert('Till km måste vara >= Från km'); return; }
    const entry={...jForm, fromKm: km(jForm.fromKm), toKm: km(jForm.toKm)};
    setJournal([...journal, entry]);
    setJForm({startDate:'',startTime:'',endDate:'',endTime:'',fromKm:'',toKm:'',arende:''});
  }

  function saveMonthOdo(){
    if(!jMonth){ alert('Välj månad'); return; }
    const meta={...monthMeta}; const cur=meta[jMonth]||{};
    const newKmIn = kmInInput!==''? km(kmInInput) : cur.kmIn;
    const newKmOut = kmOutInput!==''? km(kmOutInput) : cur.kmOut;
    meta[jMonth] = { kmIn:newKmIn, kmOut:newKmOut };
    if(newKmOut!=null){ const nx=nextMonthKey(jMonth); if(nx){ const nxMeta=meta[nx]||{}; nxMeta.kmIn=newKmOut; meta[nx]=nxMeta; } }
    setMonthMeta(meta); setKmInInput(''); setKmOutInput('');
  }

  const jSummary = jMonth ? buildJournalSummaryForMonth(jMonth, journal, monthMeta) : null;

  return e('div',{style:{padding:'20px',maxWidth:'760px',margin:'auto'}},[
    e('h1',{style:{fontSize:'26px',marginBottom:'20px'}},'Körjournal'),

    e('h2',null,'Körjournal – logg & export'),
    ...
    e('h2',null,'Tankning & Tvätt – Export'),
    Label('Välj månad'), e('select',{value:month,onChange:e=>setMonth(e.target.value)},[
      e('option',{value:''},'Välj månad...'), ...['01','02','03','04','05','06','07','08','09','10','11','12'].map(m=>e('option',{value:'2026-'+m},'2026-'+m))
    ]),
    e('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px',marginTop:'10px'}},[
      e('button',{onClick:()=>exportMonthToExcel(month,JSON.parse(localStorage.getItem('tankningar')||'[]'),JSON.parse(localStorage.getItem('tvattar')||'[]')),style:{padding:'12px',background:'#3b82f6',borderRadius:'8px'}},'Exportera Excel'),
      e('button',{onClick:()=>mailMonthExcel(month,JSON.parse(localStorage.getItem('tankningar')||'[]'),JSON.parse(localStorage.getItem('tvattar')||'[]')),style:{padding:'12px',background:'#0ea5e9',borderRadius:'8px'}},'Maila kopia')
    ])
  ]);
}
