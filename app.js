...
function App(){
  ...
  const [tankningar]=React.useState(JSON.parse(localStorage.getItem('tankningar')||'[]'));
  const [tvattar]=React.useState(JSON.parse(localStorage.getItem('tvattar')||'[]'));

  const [summaryMonth,setSummaryMonth]=React.useState(''); // för summering/översikt
  const [exportMonth,setExportMonth]=React.useState('');  // NY: separat val för export av Körjournal
  ...
  return e('div',{style:{padding:'20px',maxWidth:'780px',margin:'auto'}},[
    ...
    // Översikt / summering (egen månadsväljare)
    ...
    // NY: Körjournal – export med egen månadsväljare
    ...
    // Tankning & Tvätt export (oförändrat)
